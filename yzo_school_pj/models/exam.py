...
class SchoolExam(models.Model):
    ...
    student_id=fields.Many2one('school.attendance.line',string="Student Name",domain="[('rollcall_percent','>=',0.06),('attendance_line_id.monthly','=', monthly)]") #for student name    
    student_roll=fields.Char(string='Roll Number',readonly=True)
    section=fields.Char(string='Section',readonly=True)

    major_id=fields.Many2one('school.major',string="Subject")
    teacher_id=fields.Many2one('school.pj','Subject Teacher', readonly=True)

    # question_id=fields.Many2one('question.line',string="Question:")
    
    exam_ids=fields.One2many('exam.line','exam_id',string="Questions",store=True)
    ...
    def onchange_student_id(self):
        ...
        if self.student_id :
            ...
            self.section=self.student_id.attendance_line_id.section_name.section_name
    ...
    def add_custom_question_ids(self):
        if self.major_id:
            ...
            self.exam_ids=[(5,0,0)]
            que_ids = self.env['question.line'].search([])
            # print("....",que_ids)
            if que_ids:
                for que in que_ids:
                    # print(".....",que)
